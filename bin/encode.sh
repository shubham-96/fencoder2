...
case "$S3_KEY" in
	...
	input/av1down/*)
		CODEC="libsvtav1"
		CODEC_FLAGS="-preset 6 -crf 30 -svtav1-params tune=0:scd=1:enable-overlays=1 -pix_fmt yuv420p10le -vf scale=-2:1440,fps=fps=source_fps -fps_mode cfr"
		;;
	input/preserve/*)
		SCALE="-vf fps=fps=source_fps -fps_mode cfr"
		;;
	input/downscale/*)
		SCALE='-vf scale=-2:1440,fps=fps=source_fps -fps_mode cfr' 
		;;
	input/flip/*)
		SCALE='-vf hflip,fps=fps=source_fps -fps_mode cfr' 
		;;
	input/downflip/*)
		SCALE='-vf scale=-2:1440,hflip,fps=fps=source_fps -fps_mode cfr' 
		;;
	*)
