...
  - Use Fargate Spot exclusively, no need for standard Fargate
...
### 3. Custom Dockerfile
- **Purpose**: Create a Docker image with FFmpeg 8.0 for video encoding optimized for AWS Batch.
- **Base Image**: Use Alpine Linux for its small size (~5 MB) to minimize container pull times.
- **Dependencies**: Use a pre built ffmpeg image based on alpine & install s5cmd in it for S3 operations. 
...
  - Set a constant rate factor (CRF) of 23 for a balance between quality and file size (lower CRF = better quality, higher file size).
  - For files in `input/preserve/`, maintain original resolution.
  - For files in `input/downscale/`, downscale 4K videos to 1440p (e.g., `-vf scale=-1:1440`).
- **Upload**: Use s5cmd to upload the encoded video to `s3://bucket-name/output/`.
...
- **Runtime Prefixes**: No need to create the prefixes in code, let then be created during object uploads. 
...
   - Build the Dockerfile using Alpine Linux and FFmpeg 8.0 from source with libx265 support.
   - Push the image to a public Docker Hub repository.
1. **Set Up AWS Batch**:
   - Create a Fargate Spot compute environment. 
...
## Optimization Notes
- **Cost**: Monitor Fargate Spot costs and Docker Hub pull performance in AWS Cost Explorer. Alpine Linux minimizes container size, but verify build times for FFmpeg 8.0. Adjust vCPU/memory if over-provisioned.
